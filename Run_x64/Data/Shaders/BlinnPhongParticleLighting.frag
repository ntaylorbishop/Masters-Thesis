...
float GetTrueDepth() {
    ...
    float z_e = 2.0 * gNearPlane * gFarPlane / (gFarPlane + gNearPlane - z_n * (gFarPlane - gNearPlane));
...
void WritePixel(vec4 color) { 
	...
	//float weight = color.a * max(10e-2, min(3 * pow(10, 3), 10 / (10e-5 + pow(z / 5, 2) + pow(z / 200, 6))));
	//float weight = max(min1, color.a) * clamp(10.0 / (1e-5 + pow(z / 5, 1.0)), 1e-2, 3e3);
 	//float weight = max(min1, color.a) * clamp(0.03 / (1e-5 + pow(z / 10, 3.0) + pow(z / 200, 6.0)), 1e-2, 3e3);
	//float weight = max(min1, color.a) * clamp(0.03 / (1e-5 + pow(z / 200, 4.0)), 1e-2, 3e3);
	//float k = 0.01;
	//float weight = 10.0 / (1.0 + 10.0 * pow(z * k, 5));
...
void main() {
	...
	vec4	colorToPlace	= finalColor * passColor;

	WritePixel(colorToPlace);
}
